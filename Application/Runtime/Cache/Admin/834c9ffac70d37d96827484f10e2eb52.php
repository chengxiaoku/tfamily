    ...
    <section class="content" style="background-color: white; margin: 15px;" >
        ...
        <div class="box-header">
            <div class="pull-right">
                <form action="<?php echo U('Order/index_sel');?>" method="post">
                <label>过滤:</label>
                <span id="region">
                    ...
                    <div style="width:250px; margin-left: 5px;" class="input-group fr">
                        <input type="text" id="select" value="<?php echo ($find); ?>" placeholder="输入用户名或电话..." class="form-control" name="find">
        ...
        <div class="box-body">
            <div class="col-md-12 ">
                ...
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <a href="<?php echo U('order/house_mess',array('id_mess'=>$row['id']));?>" class="btn btn-default btn-xs">查看详情</a>
                            <a onclick="sh(this)" p="<?php echo ($row['id']); ?>"  class="btn btn-default btn-xs">审核</a>
                        </td>
...
<script type="text/javascript">
    ...
    function del_ok(){
        $("#myModal").modal('hide');
        //发送后台异步审核
        $.post("<?php echo U('order/sh');?>",{sh_id:obj_id},function(result){
            if(result == 1){
                alert('出错了!');
            }else if(result == 2){
